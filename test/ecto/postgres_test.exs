defmodule EctoQLC.Adapters.PostgresTest do
  use EctoQLC.DataCase, repo: :postgres
  ...
  describe "Ecto.Query.API" do
    ...
    test "all/1" do
      assert %Decimal{} = Repo.one(from(us in UserSession, select: avg(us.id), group_by: [us.user_id]))
    ...
    test "date_add/3", %{user: user}  do
      ...
      assert Date.diff(inserted_at, user.inserted_at) in [30, 31]
    ...
    test "datetime_add/3", %{user: user} do
      ...
      assert DateTime.diff(inserted_at, user.inserted_at, :day) in [30, 31]
    ...
    test "field/2"  do
      assert %User{} = Repo.one(where(User, [u], fragment("LENGTH(?)", field(u, :email)) == 16))
    end

    test "filter/2" do
      refute Repo.one(from u in User, select: avg(u.id) |> filter(u.id < 69))
      assert %Decimal{} = Repo.one(from u in User, select: avg(u.id) |> filter(u.id >= 69))
    end

    test "fragment/1"  do
      refute Repo.one(where(User, [u], fragment("LENGTH(?)", u.email) == 50))
      assert %User{} = Repo.one(where(User, [u], fragment("LENGTH(?)", u.email) == 16))
      assert %User{email: "user@example.com"} = User
                                                |> select([u], %{u | email: fragment("?", u.email)})
                                                |> where([u], fragment("LENGTH(?)", u.email) == 16)
                                                |> Repo.one()
    end
    ...
    test "json_extract_path/2" do
      assert ~w[localhost 0.0.0.0.0 0.0.5.0.0] == Repo.all(from(u in UserSession, select: u.meta["remote_ip"]))
    end
    ...
    test "selected_as/2", %{user: %{inserted_at: posted}} do
      ...
      assert %{posted: ^posted, sum_visits: %Decimal{}} = Repo.one(query)
    end
  ...
  describe "Ecto.Query" do
    ...
    test "distinct/3" do
      assert ~w[0.0.0.0.0 0.0.5.0.0 localhost] == Repo.all(from(us in UserSession, distinct: true, order_by: [us.meta["remote_ip"]], select: us.meta["remote_ip"]))
    ...
    test "lock/2" do
      assert [] == Repo.all(from(u in User, where: u.id == 69, lock: "FOR SHARE NOWAIT"))
    end
    ...
    test "update/3" do
      ...
      assert {1, nil} = Repo.update_all(update(User, [set: [email: "B", updated_at: ^now]]), [])
      assert [%{email: "B", updated_at: ^now}] = Repo.all(where(User, [email: "B"]))
    end
  ...
  describe "Repo" do
    ...
    test "delete!/2", %{user: %{sessions: [session | _]}} do
      assert Repo.delete!(session)
    end

    test "delete/2", %{user: %{sessions: [session | _]}} do
      assert {:ok, %UserSession{}} = Repo.delete(session)
    end
    ...
    test "insert_all/3" do
      now = DateTime.utc_now()
      assert {2, [%User{}, %User{}]} = Repo.insert_all(User, [%{inserted_at: now, updated_at: now}, %{inserted_at: now, updated_at: now}], returning: true)
    ...
    test "insert_or_update!/2" do
      changeset = User.changeset(%User{}, %{email: "body"})
      assert user = Repo.insert_or_update!(changeset)
      assert %User{email: "updated"} = Repo.insert_or_update!(User.changeset(user, %{email: "updated"}))
      changeset = User.changeset(%User{}, %{email: "body"})
      assert user = Repo.insert_or_update!(changeset)
      assert %User{email: "updated"} = Repo.insert_or_update!(User.changeset(user, %{email: "updated"}))
    end
    ...
    test "insert_or_update/2" do
      ...
      assert {:ok, %User{email: "updated"}} = Repo.insert_or_update(User.changeset(user, %{email: "updated"}))
      changeset = User.changeset(%User{}, %{email: "body"})
      assert {:ok, user} = Repo.insert_or_update(changeset)
      assert {:ok, %User{email: "updated"}} = Repo.insert_or_update(User.changeset(user, %{email: "updated"}))
    end
    ...
    test "update!/2", %{user: user} do
      changeset = User.changeset(user, %{email: "body"})
      assert %User{email: "body"} = Repo.update!(changeset)
    end

    test "update/2", %{user: user} do
      changeset = User.changeset(user, %{email: "body"})
      assert {:ok, %User{email: "body"}} = Repo.update(changeset)
    end
    ...
    test "stream/2", %{user: %{sessions: sessions}} do
      assert {:ok, ^sessions} = Repo.transaction(fn -> Enum.to_list(Repo.stream(UserSession, max_rows: 2)) end)
    end

    test "update_all/2" do
      now = DateTime.utc_now()
      assert {1, nil} == Repo.update_all(User, [set: [email: "new", updated_at: now]])
      assert [%User{email: "new", updated_at: ^now}] = Repo.all(User)
    end
  end
end
