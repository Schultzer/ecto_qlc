defmodule EctoQLC.Adapters.MnesiaTest do
  use EctoQLC.DataCase, repo: :mnesia
  ...
  describe "Ecto.Query" do
    ...
    test "lock/2" do
      assert [%User{}] = Repo.all(from(u in User, lock: "read"))
      assert [%User{}] = Repo.all(from(u in User, lock: "write"))
      assert [%User{}] = Repo.all(from(u in User, lock: "sticky_write"))
      assert_raise Ecto.QueryError, ~r/Unsupported lock: "{:global {__MODULE__, User}, read}", supported locks: write, read, stickey_write in query/, fn ->
        assert [%User{}] = Repo.all(from(u in User, lock: "{:global {__MODULE__, User}, read}"))
      end
    end
